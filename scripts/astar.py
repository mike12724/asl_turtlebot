    ...
    def get_neighbors(self, x):
        ...
        for lr in [-1,0,1]:
        	for ud in [-1,0,1]:
        		...
        		move = np.array([lr, ud])
        		move = self.resolution*move #move by self.resolution, DO WE NEED TO NORMALIZE???
        		pos = self.snap_to_grid((x[0] + move[0], x[1]+move[1]))
        		if self.is_free(pos):
        			neigh.append(pos)
        ...
        while len(self.open_set)>0:
            ...
            for neigh in neighs:
            	if neigh in self.closed_set:
            		continue
            	tg_score = self.g_score[x] + self.distance(x,neigh)
            	if neigh not in self.open_set:
            		self.open_set.append(neigh)
            	elif tg_score > self.g_score[neigh]:
            		continue
            	self.came_from[neigh] = x
            	self.g_score[neigh] = tg_score
            	self.f_score[neigh] = tg_score + self.distance(neigh,self.x_goal)
