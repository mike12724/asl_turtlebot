...
class obj_loc_broadcaster:
    ...
    def add_new_tf(self,msg):
        ...
        for i in range(self.map[obj_type]):
            ...
            self.br.sendTransform((posit.x,posit.y,posit.z),(ori.x, ori.y, ori.z, ori.w) , rospy.Time(0), obj_frame_id, self.origin_frame)
            self.trans.waitForTransform('/'+obj_type+'_'+str(i), obj_frame_id, rospy.Time(0), rospy.Duration(4.0))

            (trans,rot) = self.trans.lookupTransform('/'+obj_type+'_'+str(i), obj_frame_id, rospy.Time(0))
